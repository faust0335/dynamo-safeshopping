...
public class SafeShoppingAdapter extends AbstractService 
{
	...
	public SafeShoppingAdapter(IcapServer icapserver, Socket clientsocket){
		...
		try{
			...
			System.err.println("Safe Shopping service initialisation failure - check if language engines are correctly installed");
			e.printStackTrace();
			System.exit(1);
		}
	...
	public int getResponse(ByteArrayOutputStream bas)  {
		...
		try{
			...
			bas.reset();
			System.gc();
			Log.error(Log.WARNING,"Safe Shopping failure while doing security check: - ", e);
		}
