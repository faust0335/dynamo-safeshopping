...
public class Starter {
	private final static int DEFAULT_LOCAL_PORT = 10000;
	...
	private static Thread icapInstance = new Thread() {
		@Override
		public void run() {
			//System.err.println("icapInstance");
	...
	public static void main(String[] args) {
		...
		ca.setLocalEnforcerAddress(new InetSocketAddress("localhost", DEFAULT_ENFORCER_PORT));
		ca.setPrivateAddress(new InetSocketAddress("localhost", DEFAULT_INTERCEPT_PORT));
		try {
			coordinator = new Coordinator("test", 
					new ServerSocket(DEFAULT_LOCAL_PORT, 0, InetAddress.getByName("localhost")), 
					new ServerSocket(DEFAULT_REMOTE_PORT, 0, InetAddress.getByName("localhost")),
					ca ,
					new SafeShoppingPolicy("safePolicy"), 
					Level.DEBUG);
		} catch (IOException e) {
			e.printStackTrace();
		}
